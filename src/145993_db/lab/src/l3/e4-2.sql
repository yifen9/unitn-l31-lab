...
WITH
    tmp2 AS (
        SELECT E.employee_id, O.ship_country, (O.shipped_date - O.order_date) AS tmp
        FROM employees AS E
        ...
        WHERE (E.hire_date - E.birth_date) / 365 < 35
            ...
            AND (
                ...
                FROM orders
                WHERE
                    orders.employee_id = E.employee_id
            ) > 45
    )
SELECT employee_id, ship_country, AVG(tmp), COUNT(tmp)
FROM tmp2
GROUP BY
    employee_id,
    ship_country
